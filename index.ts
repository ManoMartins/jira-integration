...
if (program.opts().greet) {
  ...
  const name = Object.entries(mock1.names).find(([, name]) => name === 'Squad');
  const closedSprints = mock.values.filter(sprint => sprint.state === 'closed');

  const sprintsBySquad = closedSprints.reduce((acc, sprint) => {
  ...
  const lastSprintsBySquad = Object.values(sprintsBySquad).map(sprints => {
    //@ts-ignore
    sprints.sort((a, b) => new Date(b.endDate) - new Date(a.endDate));
    
    const issueSummaries = mock1.issues.map(issue => {
      // @ts-ignore
    ...
    const doc = new Document({
      sections: [{
          properties: {},
          children: lastSprintsBySquad[0].issues.map(issue => new Paragraph({
